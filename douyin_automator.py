...
class DouyinAutomator:
    ...
    def force_restart_douyin(self) -> bool:
        ...
        # 先强制停止应用
        stop_success = self.adb.force_stop_app(self.douyin_package)
        if stop_success:
    ...
    def click_element_by_text(
        self, keywords: List[str], exact_match: bool = False
    ) -> bool:
        """根据文本点击元素。
        ...
        # 对于不可点击的文本元素，尝试找到其可点击的父元素
        target_element = self._find_clickable_element(elements)

        if not target_element:
            # 尝试查找包含该文本的可点击父元素
            target_element = self._find_clickable_parent(elements)
        ...
        if center:
            element_desc = target_element.text or target_element.content_desc
    ...
    def _find_clickable_parent(self, elements):
        ...
        for text_element in elements:
            ...
            for clickable_element in self.ui_analyzer.elements:
                ...
                # 检查可点击元素是否包含文本元素的坐标范围
                if (
                    clickable_bounds[0] <= text_bounds[0]
                    and clickable_bounds[1] <= text_bounds[1]
                    and clickable_bounds[2] >= text_bounds[2]
                    and clickable_bounds[3] >= text_bounds[3]
                ):
    ...
    def navigate_to_profile(self) -> bool:
        ...
        for attempt in range(self.max_retries):
            self.logger.info("第 %d 次尝试导航到个人资料", attempt + 1)
            ...
            # 使用改进的查找逻辑
            target_element = self._find_profile_tab_smart()
            
            if target_element:
                if self._click_element(target_element):
                    # 验证是否成功导航到个人资料页面
                    time.sleep(2)
                    if self._verify_profile_page():
                        ...
                        self.logger.warning("点击成功但未到达个人资料页面")
                else:
                    self.logger.warning("点击个人资料标签失败")
            else:
                self.logger.warning("未找到个人资料标签")

            # 如果前面的方法都失败，尝试经验位置
            if attempt == self.max_retries - 1:
                self.logger.info("尝试经验位置点击")
                if self._try_experience_position_click():
                    time.sleep(2)
                    if self._verify_profile_page():
                        return True
        ...
        self.logger.error("导航到个人资料页面失败")
        return False

    def _find_profile_tab_smart(self):
        """智能查找个人资料标签"""
        # 策略1: 通过资源ID查找
        target = self._find_profile_by_resource_id()
        if target:
            return target
            
        # 策略2: 在底部导航栏查找
        profile_keywords = ["我", "Me", "Profile"]
        target = self._find_profile_by_position(profile_keywords)
        if target:
            return target
            
        # 策略3: 通过文本内容查找
        target = self._find_profile_by_text(profile_keywords)
        if target:
            return target
            
        # 策略4: 查找底部导航栏中的最右侧元素
        target = self._find_rightmost_bottom_tab()
        if target:
            return target
            
        return None

    def _find_rightmost_bottom_tab(self):
        """查找底部导航栏最右侧的标签（通常是"我"）"""
        ...
        for element in self.ui_analyzer.elements:
            if not element.is_clickable_element():
                continue
            center = element.get_center()
            if center and center[1] > screen_height * 0.8:
                bottom_elements.append(element)
        
        # 按X坐标排序，返回最右侧的元素
        if bottom_elements:
            def get_x_pos(element):
                center = element.get_center()
                return center[0] if center else 0
                
            rightmost = max(bottom_elements, key=get_x_pos)
            self.logger.info("找到底部最右侧元素")
            return rightmost
    ...
    def _verify_profile_page(self) -> bool:
        ...
        # 查找个人资料页面的特征元素
        profile_indicators = ["添加朋友", "编辑资料", "关注", "粉丝",
                              "Add Friends", "Edit Profile", "Following", 
                              "Followers"]
    ...
    def _find_profile_by_resource_id(self):
        """通过资源ID查找个人资料标签"""
        tab_resource_ids = [
            "com.ss.android.ugc.aweme:id/tab_me",
            "com.ss.android.ugc.aweme:id/me_tab",
            "com.ss.android.ugc.aweme:id/main_tab_me",
        ]

        for resource_id in tab_resource_ids:
            elements = self.ui_analyzer.find_elements_by_resource_id([resource_id])
            if elements:
                self.logger.info("通过资源ID找到标签: %s", resource_id)
                return elements[0]
        return None

    def _find_profile_by_position(self, keywords):
        """在屏幕底部查找个人资料标签"""
        screen_height = self._get_screen_height()
        if not screen_height:
            return None
        ...
        profile_elements = self.ui_analyzer.find_elements_by_text(
            keywords, clickable_only=True
        )

        for element in profile_elements:
            center = element.get_center()
            if center and center[1] > screen_height * 0.8:
                self.logger.info("在底部区域找到个人资料标签")
                return element
        return None

    def _find_profile_by_text(self, keywords):
        """通过文本内容查找个人资料标签"""
        clickable_elements = [
            e for e in self.ui_analyzer.elements if e.is_clickable_element()
        ]
        screen_height = self._get_screen_height()
        ...
        for element in clickable_elements:
            ...
            if (
                center
                and screen_height
                and center[1] > screen_height * 0.8
                and element.contains_text(keywords)
            ):
                self.logger.info("通过位置判断找到个人资料标签")
                return element
        return None
    ...
    def _try_experience_position_click(self):
        """尝试经验位置点击"""
        self.logger.warning("未找到个人资料标签，尝试经验位置点击")
        screen_size = self.adb.get_screen_size()
        if not screen_size:
            return False

        width, height = screen_size
        experience_x = int(width * 0.9)
        experience_y = int(height * 0.95)

        self.logger.info("尝试点击经验位置: (%d, %d)", experience_x, experience_y)
        success = self.adb.tap(experience_x, experience_y)
        if success:
            time.sleep(self.operation_delay)
            return True
        return False
    ...
    def follow_contact(self, contact_info: Dict) -> bool:
        ...
        if not contact_info.get("can_follow"):
            self.logger.warning("联系人 %s 无法关注", contact_info["name"])
            return False
    ...
    def _process_contact_page(
        self, contacts, start_index, max_count, processed_count, results
    ):
        """处理当前页面的联系人"""
        page_processed = 0
        for i, contact in enumerate(contacts):
            ...
            self.logger.info(
                "处理联系人 %d/%d: %s", processed_count + 1, max_count, contact_name
            )
            ...
            results["contact_details"].append({"name": contact_name, "status": status})
    ...
    def batch_follow_contacts(self, max_count: int = 10, start_index: int = 0) -> Dict:
        """批量关注通讯录联系人。
    ...
    def run_complete_workflow(self, max_follow_count: int = 10) -> Dict:
        ...
        try:
            ...
            # 步骤2: 强制重启抖音
            self.logger.info("步骤 2/6: 强制重启抖音应用...")
            if not self.force_restart_douyin():
                workflow_result["error_message"] = "抖音重启失败"
                return workflow_result
