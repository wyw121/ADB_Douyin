...
class SmartADBConnection:
    ...
    def execute_command(self,
        ...
        try:
            if self.device_id:
                full_command = (f'"{self.adb_path}" -s {self.device_id} '
                                f'{command}')
            else:
    ...
    def start_app(self, package_name: str) -> bool:
        """启动应用"""
        cmd = (f"shell monkey -p {package_name} "
    ...
    def is_app_running(self, package_name: str) -> bool:
        """检查应用是否运行"""
        result = self.execute_command(f"shell ps | grep {package_name}")
        return result["success"] and package_name in result["output"]
...
class SmartTextMatcher:
    ...
    def __init__(self):
        ...
        self.keyword_variants = {
            "我": ["我", "鎴?", "Me"],
            "添加朋友": ["添加朋友", "添加好友", "加朋友", "娣诲姞鏈嬪弸", "Add Friends"], 
            "通讯录": ["通讯录", "联系人", "通信录", "閫氳褰?", "鎵嬫満閫氳褰?", "Contacts"],
            "关注": ["关注", "+关注", "鍏虫敞", "Follow"],
            "返回": ["返回", "后退", "杩斿洖", "Back", "←", "<"],
            "搜索": ["搜索", "鎼滅储", "Search"]
        }
...
class SmartUIAnalyzer:
    ...
    def parse_xml(self,
                  xml_content: str,
                  screen_size: Tuple[int, int] = None) -> bool:
        """解析XML内容"""
        try:
    ...
    def find_elements_by_keyword(self,
                                 keyword: str,
    ...
    def find_bottom_navigation_element(self,
                                       keyword: str
                                       ) -> Optional[SmartUIElement]:
        """查找底部导航元素"""
    ...
    def find_best_element_by_keyword(self,
                                     keyword: str
                                     ) -> Optional[SmartUIElement]:
        """找到最佳匹配的元素"""
        candidates = self.find_elements_by_keyword(keyword)
