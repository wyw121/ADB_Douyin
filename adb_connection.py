...
class ADBConnection:
    ...
    def start_app(self, package_name: str, activity_name: str = None) -> bool:
        ...
        if activity_name:
            ...
            command = (f"shell monkey -p {package_name} "
                       f"-c android.intent.category.LAUNCHER 1")
        
        result = self.execute_command(command)
        if result['success']:
            self.logger.info(f"启动应用 {package_name} 成功")
            return True
        else:
            self.logger.error(f"启动应用失败: {result['error']}")
            return False
    
    def wait_and_retry(self, operation, max_retries: int = 3,
                       delay: float = 1.0):
        """
        带重试的操作执行
        ...
        for attempt in range(max_retries):
            try:
                ...
                warning_msg = (f"操作失败 (尝试 {attempt + 1}/"
                               f"{max_retries}): {str(e)}")
        ...
        self.logger.error(f"操作失败，已重试 {max_retries} 次")
        return None
